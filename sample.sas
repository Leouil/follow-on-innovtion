...
*-1) keep sample years from 1999 to 2017 covered by Compustat and CRSP;
data controls;
  set controls;
  if missing(permno)=0;
  *# if cik=. then delete;
...
*#-2) aggregate patent citation to cited-citing firm-pairs;
*## excluding self-citations, citations when the cited and citing patents share common inventors, citations added by patent examiners, and citations from patents whose age is greater than twenty years.;
data allcitations;
...
*### add technology similarity - cosine similarity computed following Jaffe (1986);
proc sql;
create table paircite as
select distinct
...
*#-3) add transparency measures (i.e., dq and R&D disclosure) and control variables;
proc sql;
...
%let winvars = &controls &patvars logcitenumadj
...
*#-1) sample year from 1999 to 2017 covered by Compustat and CRSP;
data controls;
  set controls;
  if missing(permno)=0;
  if fyear>=1999 and fyear<=2017;
...
proc sql;
*## add transparency measures (generated by trans.sas);
create table sample as 
select distinct
...
proc sql;
*# aggregate from paircite sample, add transparency measures;
create table paircite as 
select distinct
...
proc sql;
*# add DAPC and LogPATME following Hirshleifer et al. (2013);
create table citingfirms as
select distinct
